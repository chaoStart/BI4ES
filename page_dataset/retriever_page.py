...
index_name = "ragflow_sciyonff8bcdc11efbdcf88aedd6333bi_page"
...
def hybrid_search_indicator(query_text, query_vector, id="690682229338275840"):
    # ====== 1. 关键词检索 ======
    keyword_query = {
        "bool": {
            "must": [
...
# 检索ES数据
results = hybrid_search_indicator(query_text, query_vector, id="690682229338275840")
...
for v in high_indicator_res:
    n = v["page_name"]
    page_id = v["page_id"]
    des = v["page_description"]
    print(f"数据集名称Name: {n} - 数据集ID:{page_id} - 数据集描述Description: {des}")
