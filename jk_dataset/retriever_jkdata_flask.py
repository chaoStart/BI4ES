...
def hybrid_search_indicator(query_text, query_vector, id="691222347303550994", es_index_name="ragflow_sciyonff8bcdc11efbdcf88aedd6333bi_jkdata" ):
    # ====== 1. 关键词检索 ======
    keyword_query = {
        "bool": {
            "must": [
    ...
    for key, val in merged.items():
        ...
        final_results.append({
            "score": score,
            "dataset_id": val["source"]["id"],
            "dataset_name": val["source"]["name"],
...
@app.route('/search_jkdata', methods=['POST'])
def search():
    try:
        data = request.get_json()
        query_text = data.get("query_text")
        dataset_id = data.get("dataset_id", "691222347303550994")
        ...
        # 执行混合检索
        results = hybrid_search_indicator(query_text, query_vector, dataset_id)
