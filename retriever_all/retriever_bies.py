...
def normalization(sres):
    ...
    if max_score != min_score:
        for item in sres:
            item["normalized_score"] = (item["_score"]-min_score)/scale
    else:
        for item in sres:
...
def get_high_score(results_list):
    ...
    for index, item in enumerate(results_list):
        ...
        if (score1 - score_x) / score1 <= 1.0:
            record_high_score.append(item)
    return record_high_score


def hybrid_search_indicator(query_text, query_vector, kb_id):
    # ====== 1. 关键词检索 ======
    keyword_query = {
        "bool": {
            "must": [
                {
                    "multi_match": {
                        "query": query_text,
                        "fields": ["name^3", "description^2", "main_column_info"],
                        "type": "best_fields",
                        "analyzer": "ik_smart"
                    }
                }
    ...
    keyword_res = es.search(index=index_name, query=keyword_query, size=30)["hits"]["hits"]
    # 调用归一化方法
